...
const APPLY_SH = path.join(ROOT, 'actions', 'terraform-apply', 'scripts', 'apply.sh');
const ACTION = path.join(ROOT, 'actions', 'terraform-apply', 'action.yml');
...
test('apply.sh captures terraform own exit code via PIPESTATUS, not the pipeline/tee status', () => {
...
test('action wires the error into the failure notification and exports it', () => {
...
test('a successful apply exits 0 and publishes no error detail', () => {
...
test('an error containing the heredoc delimiter cannot forge output entries', () => {
  const sh = fs.readFileSync(APPLY_SH, 'utf8');
