#!/usr/bin/env bash
# Run terraform apply against the saved plan, capture ITS OWN exit code, and extract the
# underlying error so the failure notification can carry it. Extracted from action.yml so it can
# be shellcheck'd and unit-tested directly, mirroring actions/terraform-apply/scripts/apply.sh.
#
